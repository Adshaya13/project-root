...
export default function StaffBookingHistoryPage() {
  const { bookings, handleCancelBooking, bookingHistory } = useStaffBooking();
  const now = new Date();
  ...
  const completedThisMonthCount = bookings.filter((booking) => {
    if (booking.status !== "APPROVED") {
      return false;
    }

    return booking.bookingDate.startsWith(monthPrefix) && booking.bookingDate <= todayKey;
  }).length;

  const cancelledLast30DaysCount = bookings.filter((booking) => {
    if (booking.status !== "CANCELLED") {
      return false;
    }

    const changedAt = booking.cancelledAt ?? booking.updatedAt;
    const changedDate = new Date(changedAt);
    return !Number.isNaN(changedDate.getTime()) && changedDate >= thirtyDaysAgo;
  }).length;
      ...
      <div className="flex min-h-full flex-col gap-6">
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600 sm:text-base">
                Review past bookings, cancellations, and completed requests.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-3 font-display text-4xl font-semibold text-slate-900">{String(completedThisMonthCount).padStart(2, "0")}</div>
            <div className="mt-2 text-sm text-slate-500">This month</div>
          </article>
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="text-sm text-slate-500">Cancelled</div>
            <div className="mt-3 font-display text-4xl font-semibold text-slate-900">{String(cancelledLast30DaysCount).padStart(2, "0")}</div>
            <div className="mt-2 text-sm text-slate-500">Last 30 days</div>
          </article>
          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="text-sm text-slate-500">Rejected</div>
            <div className="mt-3 font-display text-4xl font-semibold text-slate-900">{String(rejectedCount).padStart(2, "0")}</div>
            <div className="mt-2 text-sm text-slate-500">Declined requests</div>
          </article>
        </section>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 overflow-x-auto">
            ...
            {bookingHistory.length === 0 && (
              <div className="py-8 text-sm text-slate-500">No booking history.</div>
            )}
          </div>
        </section>
      </div>
